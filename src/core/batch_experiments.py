...
def _summarize_single_case(results: Dict[str, Any], args: SimpleNamespace) -> Dict[str, Any]:
    ...
    return {
        ...
        "low_level_planner": results.get("low_level_planner", getattr(args, "low_level_planner", PAPER_LOW_LEVEL_PLANNER)),
        "random_seed": results.get("random_seed", getattr(args, "seed", None)),
...
def run_batch_simulation(args: SimpleNamespace) -> Dict[str, Any]:
    ...
    for case_number in get_case_numbers():
        ...
        case_args.low_level_planner = PAPER_LOW_LEVEL_PLANNER
