...
可选地附加一行 `LLM-baseline`：
  ...
  - 关闭机动记忆
  - 关闭规则验证器
  - 用于近似“在 baseline 的反应式框架上仅叠加固定频率 LLM 决策”的对比设定

对比指标：
  - R_max  : 场景内最大碰撞风险（越低越好）
  - R_avg  : 场景内平均碰撞风险（越低越好）
  - A_turn : LLM与纯反应式基准的转向一致率（越高越好）
  - ΔD     : 路径效率偏差（相对基准，越小越好）
...
def _extract_metrics(
    ...
    return {
        ...
        "risk_threshold": results.get("llm_risk_threshold", 0.30),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--risk_threshold", type=float, default=0.30)
    ...
    p.add_argument("--llm_baseline_interval", type=int, default=200,
...
def main() -> None:
    ...
    extra_methods = 1 if include_llm_baseline else 0
